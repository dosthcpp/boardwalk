          ...
          children: [
                    ...
                    child: MaterialButton(
                      ...
                      onPressed: () async {
                        try {
                          ...
                          if (account != null) {
                            var info = await _authServiceThirdParty
                                .getInfo(account.id);
                            if (info == 'no_info') {
                              AwesomeDialog(
                                context: context,
                            ...
                            onPressed: () async {
                              try {
                                if (await apple.SignInWithApple.isAvailable()) {
                                  ...
                                  var info = await _authServiceThirdParty
                                      .getInfo(credential.userIdentifier);
                                  if (info == 'no_info') {
                                    AwesomeDialog(
                                      context: context,
                                      animType: AnimType.SCALE,
            ...
            SizedBox(
              ...
              child: MaterialButton(
                ...
                onPressed: () async {
                  // await _authService.test(userProvider.getEmail());
                  var hasSignedIn = await _authService.checkSignedIn(userProvider.getEmail());
                  if(hasSignedIn == null) {
                    setState(() {
                    ...
                    if(hasSignedIn) {
                      setState(() {
                        status = 'This user has already signed in.';
                      });
                      ...
                      String message = await _authService.signIn(_password);
                      if (message == 'ok') {
